                ...
                <el-row :gutter="gutterSize">
                  ...
                  <el-col :span="9">
                    <el-form-item :label="$t('message.roleInfo.roleName')">
                      <el-input
                        v-model="parameter.roleName"
                        :placeholder="$t('message.roleInfo.roleNamePlaceholder')"
                      ></el-input>
                    </el-form-item>
                  </el-col>
                  <el-col :span="6">
          ...
          <el-collapse-item name="result">
            ...
            <div class="home-subView-result-style">
              ...
              >
                ...
                <el-table-column
                  prop="roleDesc"
                  :label="$t('message.roleInfo.roleDesc')"
                  show-overflow-tooltip
                >
                </el-table-column>
                <el-table-column
                  prop="firstCreateUser"
                  :label="$t('message.firstCreateUser')"
                >
                </el-table-column>
                ...
                <el-table-column
                  prop="lastUpdateUser"
                  :label="$t('message.lastUpdateUser')"
                >
                </el-table-column>
        ...
        >
          <el-row :gutter="gutterSize">
            ...
            <el-col :span="11">
              ...
              >
                <el-select
                  v-model="dialogData.roleDesc"
                  clearable
                  :placeholder="$t('message.placeholder')"
                  style="width: 100%"
                >
                  <el-option
                    v-for="item in roleDescList"
                    :key="item.contentKey"
                    :label="item.contentValue"
                    :value="item.contentKey"
                  ></el-option>
...
export default {
  ...
  methods: {
    ...
    methodPageList () {
      // 分页查询方法
      this.axios.post(this.commonConstants.url.pageRoleList, this.parameter, this.headers
      ).then((response) => {
    ...
    methodI18nList (flag) {
      // 动态获取角色相关的国际化数据
      let param = {typeCode: this.userInfo.locale, contentUsed: 'N'}
      if (flag === 'ROLE-NAME') {
        param.contentGroup = 'ROLE-NAME'
      } else {
        param.contentGroup = 'ROLE-DESC'
      }
      this.axios.post(this.commonConstants.url.languageFind, param, this.headers
      ).then((response) => {
        console.info(`【角色信息管理页面之methodRoleNameList函数】【打印角色相关的国际化数据】`, response.data.dataList)
        if (flag === 'ROLE-NAME') {
          this.roleNameList = response.data.dataList
        } else {
          this.roleDescList = response.data.dataList
        }
      })
    },
    methodQuery () {
    ...
    methodDialogOpen () {
      // 对话框打开之前，先从后台请求数据
      this.methodI18nList('ROLE-NAME')
      this.methodI18nList('ROLE-DESC')
      if (this.dialogStatus === this.commonConstants.operation.create) {
        // 打开创建界面
        this.dialogDisabled = false
        this.dialogTitle = this.$t('message.titleCreate').format(this.$t('message.roleInfo.title'))
        this.dialogShow = true
      } else {
        // 打开编辑界面，编辑角色和权限之间的对应关系
        this.dialogTitle = this.$t('message.titleUpdate').format(this.$t('message.roleInfo.title'))
    ...
    methodDialogSave () {
      if (this.dialogStatus === this.commonConstants.operation.create) {
        this.axios.post(this.commonConstants.url.addRoleInfo, this.dialogData, this.headers
        ).then((response) => {
          ...
          if (status === this.commonConstants.status.success) {
            ...
            this.$alert(this.$t('message.alert.repeatAccount'), this.$t('message.msgWarn'), {type: 'warning'})
          } else {
            this.$alert(this.$t('message.GEWEI_10001'), this.$t('message.msgWarn'), {type: 'warning'})
          }
        })
      } else {
        // 保存修改的用户数据
