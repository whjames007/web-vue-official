<template>
    <div id="GeweiSubviewBottom">
        <el-row>
            <el-col :span="24">
                <div class="home-subView-bottom-style">
...
export default {
  ...
  mounted () {
    this.getMenuLevelObject()
  },
  methods: {
    getMenuLevelObject () {
      let letThis = this
      let mlo = this.$store.getters['commonModule/getMenuLevelObject']
      if (!mlo.one) {
        console.info(`底部面包屑延迟获取`)
        setTimeout(function () {
          letThis.getMenuLevelObject()
        }, 1000)
      } else {
        this.menuLevelObject = mlo
      }
    }
  }
}
